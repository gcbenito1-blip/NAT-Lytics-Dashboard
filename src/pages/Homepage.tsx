...
import { lsGetAll } from '../services/sessionService';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full p-8 border border-slate-100">
                ...
                <div className="flex gap-3">
                    <button onClick={onClose} className="flex-1 px-4 py-2.5 border border-slate-200 rounded-xl text-slate-600 text-sm font-medium hover:bg-slate-50 transition">
                        Cancel
                    </button>
                    <button onClick={() => navigate('/dashboard')} className="flex-1 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold transition flex items-center justify-center gap-2">
...
function StepCard({ step, icon, title, description }: { step: number; icon: string; title: string; description: string; }) {
    return (
        <div className="relative flex flex-col items-center text-center group">
            ...
            <span className="inline-block text-xs font-bold text-blue-500 tracking-widest uppercase mb-1">Step {step}</span>
...
function PreviewCard({ icon, title, description, color }: { icon: string; title: string; description: string; color: 'blue' | 'violet' | 'amber' | 'emerald'; }) {
    const palette = {
    ...
    const iconBg = { blue: 'bg-blue-100', violet: 'bg-violet-100', amber: 'bg-amber-100', emerald: 'bg-emerald-100' }[color];
    return (
        <div className={`bg-gradient-to-br ${palette} border rounded-2xl p-5 flex flex-col gap-3 opacity-80`}>
...
function AdminCard({ icon, title, description, route }: { icon: string; title: string; description: string; route: string; }) {
    const navigate = useNavigate();
    return (
        <button onClick={() => navigate(route)} className="bg-white border border-slate-200 rounded-2xl p-6 flex flex-col items-center text-center hover:shadow-lg hover:border-blue-200 transition group">
...
export function Homepage() {
    ...
    useEffect(() => {
        ...
        const sessions = lsGetAll(user.id);
        if (sessions.length > 0 && location.pathname !== '/homepage') {
            navigate('/overview', { replace: true });
        }
    }, [user?.id, loading, navigate]);

    const adminCards = [
        { icon: 'manage_accounts', title: 'Manage Teachers', description: 'Add, view, and manage teacher accounts in your organization.', route: '/manage-teachers' },
        { icon: 'analytics', title: 'List of Results', description: 'View prediction history and performance results.', route: '/results-list' },
    ];

    const steps = [
        { icon: 'upload_file', title: 'Upload Class Data', description: 'Import your student CSV file with grades and demographics' },
        { icon: 'analytics', title: 'System Analyzes', description: 'The model processes student performance data automatically' },
        { icon: 'dashboard', title: 'View Overview', description: 'Explore class-wide results and performance summaries' },
        { icon: 'person_search', title: 'Identify Students', description: 'Pinpoint learners who need additional support' },
    ];
                ...
                <>
                    ...
                    <section className="max-w-4xl mx-auto px-6 py-16">
                        <div className="text-center mb-10">
                            ...
                            <p className="text-slate-500 text-sm mt-2">Manage your organization's teachers and view prediction history</p>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-2xl mx-auto [&>*]:cursor-pointer">
                            {adminCards.map((card) => <AdminCard key={card.title} {...card} />)}
                        </div>
                    </section>
                </>
            ) : null}
                ...
                <>
                    ...
                    <section className="relative overflow-hidden bg-gradient-to-br from-blue-700 via-blue-600 to-blue-800 text-white">
                        ...
                        <div className="relative max-w-4xl mx-auto px-6 py-20 text-center">
                            ...
                            <p className="text-blue-100 text-lg max-w-xl mx-auto mb-10 leading-relaxed">
                                A system that helps you understand your class performance and identify students who need support.
                            </p>
                            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                                <button onClick={() => setShowModal(true)} className="inline-flex items-center gap-2 bg-white text-blue-700 hover:bg-blue-50 px-7 py-3.5 rounded-xl font-bold text-sm shadow-lg shadow-blue-900/30 transition">
                    ...
                    <section className="max-w-4xl mx-auto px-6 py-16">
                        <div className="text-center mb-12">
                            <p className="text-xs font-bold tracking-widest text-blue-500 uppercase mb-2">Getting Started</p>
                    ...
                    <section className="bg-white border-y border-slate-100">
                        <div className="max-w-4xl mx-auto px-6 py-16">
                            <div className="text-center mb-10">
                                <p className="text-xs font-bold tracking-widest text-blue-500 uppercase mb-2">File Requirements</p>
                                <h2 className="text-2xl font-extrabold text-slate-800">Accepted File Format</h2>
                                <p className="text-slate-500 text-sm mt-2">
                                    Upload a <span className="font-semibold text-slate-700">.csv</span> file with the following columns:
                                </p>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                {requiredColumns.map((col) => (
                                    <div key={col.label} className="flex items-center gap-3 bg-slate-50 border border-slate-100 rounded-xl px-4 py-3">
                    ...
                    <section className="max-w-4xl mx-auto px-6 py-16">
                        <div className="text-center mb-10">
                            <p className="text-xs font-bold tracking-widest text-blue-500 uppercase mb-2">After Upload</p>
                            <h2 className="text-2xl font-extrabold text-slate-800">What You Will See</h2>
                            <p className="text-slate-500 text-sm mt-2">Once your data is uploaded, you'll have access to these features</p>
                        </div>
